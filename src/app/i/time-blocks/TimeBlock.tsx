...
import { useDeleteTimeBlock } from '@/hooks/time-blocks/useDeleteTimeBlock'
import { useTimeBlocksSortable } from '@/hooks/time-blocks/useTimeBlocksSortable'

export function TimeBlock({ item }: { item: ITimeBlockResponse }) {
	const { attributes, style, setNodeRef, listeners } = useTimeBlocksSortable(
		item.id
	)
